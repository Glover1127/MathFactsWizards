...
if st.session_state.current_level is None:
    ...
    if st.button("Start Game"):
        ...
        st.session_state.score = 0
        st.session_state.current_problem = st.session_state.problems[0] if st.session_state.problems else None
    # When no level is chosen yet, we display only the level-selection UI.
else:
    # Main Game Screen
    ...
    # Display the current problem using the stored value.
    ...
    if submitted and st.session_state.current_problem:
        # Use local variables for the current problem
        ...
        # Remove the current problem from the deck since it's been answered.
        if st.session_state.problems:
            st.session_state.problems.pop(0)
        
        # Update current_problem to the next one in the deck, if available.
        if st.session_state.problems:
            st.session_state.current_problem = st.session_state.problems[0]
        else:
            st.session_state.current_problem = None
        
        # Check if score conditions require a level reset or advancement.
        ...
        # Force a rerun to immediately update the UI with new question and score.
        st.experimental_rerun()
